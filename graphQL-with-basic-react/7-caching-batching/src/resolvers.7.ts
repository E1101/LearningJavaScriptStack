...
async function getArtist(
  ...
  const artists = await loaders.artist.load(artistId);

  return Object.keys(artists).reduce(
    (result, key) => ({
      ...result,
      [toCamelCase(key)]: artists[key],
    }),
    {},
  );
}
...
async function getTracksByArtist(
  ...
  if (
    info.path.prev &&
    info.path.prev.prev &&
    info.path.prev.prev.prev &&
    info.path.prev.prev.prev.prev &&
    info.path.prev.prev.prev.prev.prev
  ) {
    throw new Error('This operation is too deeply nested');
  }
